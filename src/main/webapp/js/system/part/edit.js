...
function byPart(state){
    ...
    if (data != null) {
        ...
        if(parseInt(state,10)==1){
            h+="<option value='" + 1 +"' selected='selected'>正常</option>";
            h+="<option value='" + 0 +"'>停用</option>";
        }else{
            h+="<option value='" + 1 +"'>正常</option>";
            h+="<option value='" + state +"' selected='selected'>停用</option>";
        }
        $("#state").html(h);
    } else {
        bootbox.alert("获取状态信息错误，请联系管理员！");
    }
}