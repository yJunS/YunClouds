				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<input type="text" class="form-control checkRole"
							placeholder="请输入角色名" name="roleFormMap.roleName" id="roleName" value="${role.roleName}"/>
					</div>
				</div>
					...
					<div class="col-sm-9">
						<div class="btn-group m-r">
							<button data-toggle="dropdown"
								...
								<span class="dropdown-label"><c:if test="${role.state eq 0}">可用</c:if><c:if test="${role.state eq 1}">不可用</c:if></span> <span class="caret"></span>
							</button>
							<ul class="dropdown-menu dropdown-select">
								<li class=""><a href="#"><input type="radio"
										name="roleFormMap.state" value="0"<c:if test="${role.state eq 0}"> checked="checked"</c:if>>可用</a></li>
								<li class="active"><a href="#"><input type="radio"
										name="roleFormMap.state" value="1" <c:if test="${role.state eq 1}"> checked="checked"</c:if>>不可用</a></li>
							</ul>
						</div>
					</div>
				</div>
